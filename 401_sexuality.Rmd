...
ord_model1 <- polr(scorecat ~ num_tests + politicalid + age + 
                           birthSex + race + warmth_gayleswomen + warmth_gaymen + warmth_straightmen, 
...
# Fit ordinal logistic regression model
ord_model2 <- polr(scorecat ~ politicalid + age + 
                           birthSex + warmth_gayleswomen + warmth_gaymen + warmth_straightmen, 
                  data = sexualitydata, Hess = TRUE)
...
# Fit ordinal logistic regression model
ord_model3 <- polr(scorecat ~ politicalid + 
                            warmth_gayleswomen + warmth_gaymen + warmth_straightmen, 
                  data = sexualitydata, Hess = TRUE)
...
# Fit ordinal logistic regression model
ord_model4 <- polr(scorecat ~ politicalid + 
                             warmth_gaymen + warmth_straightmen, 
                  data = sexualitydata, Hess = TRUE)
...
AICs: 115275.54, 115431.05, 116039.31, 116465.60, 115251.54 (lasso)
Ord model 5 has the lowest AIC value. 
Ord model 4 has the highest AIC value. 

```{r}
#table 
ctable <- coef(summary(ord_model5))
...
ci <- confint(ord_model5)
ci
```

```{r}
#Odds Ratio and confidence intervals 
exp(cbind(OR = coef(ord_model5), ci))
```
...
# Predict classes
predicted_classes <- predict(ord_model5, newdata = test_data, type = "class")
